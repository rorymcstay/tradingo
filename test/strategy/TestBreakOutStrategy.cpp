...
TEST(BreakOutStrategy, smoke_test)
{
    ...
    env >> "NONE" LN;
    env << "QUOTE askPrice=101.0 askSize=100.0 bidPrice=100.0 bidSize=1000.0 symbol=XBTUSD" LN;
     env >> format("ORDER_CANCEL Price=100 OrderQty=0 CumQty=0 LeavesQty=0 OrderID=2 ClOrdID=MCST1 OrigClOrdID= OrdStatus=Canceled Side=Buy symbol=XBTUSD", order2)+ LN;
    auto order3 = env >> "ORDER_NEW Price=101 OrderQty=100 CumQty=0 LeavesQty=100 OrderID=6 ClOrdID=MCST3 OrigClOrdID= OrdStatus=New Side=Sell symbol=XBTUSD" LN;
    env >> "NONE" LN;
