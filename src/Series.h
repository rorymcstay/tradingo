...
public:
    ...
    class iterator_type {
        ...
        std::vector<long>::const_iterator _index_iter;
    ...
    public:
        iterator_type(
                std::vector<long>::const_iterator index_,
        ...
        iterator_type(const iterator_type& iterator_type_) {
            this->_index_iter =  iterator_type_._index_iter;
            this->_series = iterator_type_._series;
        }
        iterator_type operator++() { _index_iter++; return *this; }
        bool operator!=(const iterator_type & other) const { 
            return _series->_data[*_index_iter]->getTimestamp() != other->getTimestamp(); 
        }
        const T& operator*() const { return *_series->_data[*_index_iter]; }
...
private:
    long index_of(const utility::datetime& date_) const;
...
Series<T>::Series(const std::string& dataFile_, int resolution_) {
    ...
    auto size = _end_interval - _start_interval + 1;
    _data = std::vector<std::shared_ptr<T>>(size, nullptr);
    for (auto item : tmpdata) {
        long index = item->getTimestamp().to_interval() - _start_interval;
        _data[index] = item;
...
template <typename T>
long Series<T>::index_of(const utility::datetime& time_) const {
    if (time_.to_interval() < _start_interval) {
        throw std::runtime_error("time out of bounds");
    } else if (time_.to_interval() > _end_interval) {
        throw std::runtime_error("time out of bounds");
    }
    auto index = std::min(time_.to_interval() - _start_interval, _data.size() - 1);
    while (index >= 0) {
        if (_data.at(index))
            return index;
        index--;
...
template <typename T> long Series<T>::size() const { return _index.size(); }
